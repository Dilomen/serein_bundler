...
const { dBUtils, camlCaseObj, SocketHandler, uniteProjectBranch, INTERRUPT, UPDATE_VIEW, UPDATE_LIST_VIEW, BUILD_TYPE, TYPE_FINISH_SEND, getStatusShow } = require('../../utils')
  ...
  async searchDetail () {
    ...
    if (results.length > 0) {
      ...
      try {
        ...
        commitContent = fs.readFileSync(path.resolve(process.cwd(), `./storage/build_log/${resResult.createTime.slice(0, 10)}/${projectName}/${resResult.soloId}.log`), 'utf-8')
      } catch (err) {
        commitContent = ''
      }

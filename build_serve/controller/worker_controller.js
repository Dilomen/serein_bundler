...
let _cluster = null
const { TYPE_ADD_BUILD, TYPE_FINISH_BUILD, TYPE_FINISH_SEND, TYPE_FILECACHE_ADD, FILECACHE, INTERRUPT, TASKNOTICE, UPDATE_DIED_PROCESS_TASK_STATUS } = require('../../utils')
...
class WorkManagerController {
  ...
  static initServiceWorker () {
    ...
    // serviceWorker = childProcess.fork(path.resolve(process.cwd(), './bundler/app.js'), [], { execArgv: ['--inspect=8081'] })
    serviceWorker = childProcess.fork(path.resolve(process.cwd(), './bundler/app.js'))

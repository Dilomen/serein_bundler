...
const threadServicePath = path.resolve(__dirname, './thread_service.js')
const buildThreadMap = new Map()
const { dBUtils, encryption, uniteProjectBranch, UPDATE_DETAIL, UPDATE_VIEW, UPDATE_LIST_VIEW, TYPE_ADD_BUILD, TYPE_FINISH_BUILD, TYPE_FILECACHE_ADD, TYPE_FINISH_SEND, BUILD_TYPE } = require('../../utils')
...
class BuildService {
    ...
    execStdListening (msg) {
        this.cwdOutput += msg
        // 防止密码泄露
        this.cwdOutput = this.cwdOutput.replace(/https*:\/\/\S+\.git\/*/, 'your repository url')
    }
    ...
    async updateStatus (status, useTime) {
        ...
        if (status === BUILD_TYPE.BUILD_SUCCESS || status === BUILD_TYPE.BUILD_FAIL) {
            ...
            const dateCommitConentPath = path.join(process.cwd(), `./storage/build_log/${dayjs(new Date()).format('YYYY-MM-DD')}`)
